...
class Metabox_Settings {
	...
	public function metabox_scripts_styles( $hook_suffix ) {
		...
		// SRL: THESE SCRIPTS ONLY LOADE ON THE GALLERY, ALBUM AND TEMPLATE SETTINGS PAGE.
		if ( isset( $_GET['page'] ) && 'template_settings_page' === $_GET['page'] || $this->main_post_type === $current_info['post_type'] && 'post' === $current_info['base'] && in_array( $hook_suffix, array( 'post.php', 'post-new.php' ) ) ) {
			...
			// Register Metabox JS.
			wp_register_script( 'slick-metabox-js', plugins_url( 'feed-them-gallery/metabox-settings/js/metabox.js' ), array(), FTG_CURRENT_VERSION, true );
			...
			wp_register_script( 'updatefrombottom-admin-js', plugins_url( 'feed-them-gallery/metabox-settings/js/update-from-bottom.js' ), array( 'jquery' ), FTG_CURRENT_VERSION, true );
