...
class Shortcode_Button {
	 ...
	 * @since 1.0.0
	 */
	public function ft_gallery_editor_get_galleries() {
		global $post;
	...
	public function ft_gallery_print_media_templates() {
				...
				<div class="attachments-browser ft-gallery ft-gallery-editor">
					...
					<div class="media-toolbar">
						...
						<div class="media-toolbar-primary search-form">
							<label for="ft-gallery-search" class="screen-reader-text"><?php esc_html_e( 'Search', 'feed-them-gallery' ); ?></label>
							<input type="search" placeholder="<?php esc_html_e( 'Search', 'feed-them-gallery' ); ?>" id="ft-gallery-search" class="search"/>
						</div>
					</div>
				</div>
			</div>
		...
		<script type="text/html" id="tmpl-ft-selection-sidebar">
			<h3><?php esc_html_e( 'Helpful Tips', 'feed-them-gallery' ); ?></h3>
			<strong><?php esc_html_e( 'Choosing Your Gallery', 'feed-them-gallery' ); ?></strong><p>
				<?php esc_html_e( 'Simply click on one of the boxes to the left or you can Ctrl(PC) / cmd(MAC) and click to select multiple Galleries.  The "Insert" button will be activated once you have selected a gallery.', 'feed-them-gallery' ); ?>
			</p><strong><?php esc_html_e( 'Insert Your Gallery', 'feed-them-gallery' ); ?></strong><p>
				<?php esc_html_e( 'To insert your gallery, click on the "Insert" button below.', 'feed-them-gallery' ); ?>
			</p>
			<h3><?php esc_html_e( 'Title Options', 'feed-them-gallery' ); ?></h3>
			<p><?php esc_html_e( 'Add the Gallery Title before each shortcode and Align the Title if you like.', 'feed-them-gallery' ); ?></p>
			<div class="settings">
				<label class="setting">
					<p>
						<span class="name"><?php esc_html_e( 'Display Title', 'feed-them-gallery' ); ?></span>
						<select name="title" size="1">
							<option value="0" selected><?php esc_html_e( 'No', 'feed-them-gallery' ); ?></option>
							<?php
							for ( $i = 1; $i <= 6; $i++ ) {
								?>
								<option value="h<?php echo esc_attr( $i ); ?>"><?php echo sprintf( esc_html( 'Yes, as Heading H%s', 'feed-them-gallery' ), esc_attr( $i ) ); ?></option>
								<?php
							}
							?>
						</select>
				...
				<label class="setting">
					<span class="name"><?php esc_html_e( 'Align Title', 'feed-them-gallery' ); ?></span>
					<select name="align" size="1">
						<option value="" selected><?php esc_html_e( 'No', 'feed-them-gallery' ); ?></option>
						<option value="left"><?php esc_html_e( 'Left', 'feed-them-gallery' ); ?></option>
						<option value="center"><?php esc_html_e( 'Center', 'feed-them-gallery' ); ?></option>
						<option value="right"><?php esc_html_e( 'Right', 'feed-them-gallery' ); ?></option>
					</select>
				</label>
			</div>
		</script>
