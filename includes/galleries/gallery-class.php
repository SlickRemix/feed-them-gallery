					...
					<div class="plupload-upload-uic hide-if-no-js
						...
						<?php if ( $width && $height ) : ?>
							...
							<span class="plupload-width"
								  id="plupload-width<?php echo $width; ?>"></span>
							<span class="plupload-height"
								  id="plupload-height<?php echo $height; ?>"></span>
						<?php endif; ?>
				...
				<ul class="plupload-thumbs
					...
					if ( is_array( $image_list ) && isset( $image_list[0] ) ) {
						...
						foreach ( $image_list as $key => $image ) {
							...
							// $meta_box .= '</a>';
							$meta_box .= '<div class="ft-gallery-edit-thumb-btn"><button type="button" title="Edit this image." class="ft-gallery-edit-img-popup" data-id="' . $image['id'] . '" data-nonce="' . wp_create_nonce( 'ft_gallery_edit_image_nonce' ) . '" data-imageurl="' . $image_source_popup . '"></button></div>';
				...
				if ( ! isset( $image_list[0] ) ) {
					...
					<div class="gallery-edit-question-message gallery-edit-question-download-gallery gallery-quick-guide-getting-started">
						...
						<div id="slickdocit-show-video"
							 class="slickdocit-show-video"><?php _e( 'View Quick Setup Video', 'feed-them-gallery' ); ?>
							<span class="slickdocit-play"></span></div>
						<div id="slickdocit-hide-video"
							 class="ftg-close-vid"><?php _e( 'Close Video', 'feed-them-gallery' ); ?>
						...
						<ul id="ft-gallery-zip-list" class="ftg-free-list">
							<li class="ft-gallery-zip zip-list-item-24527">
								<div class="ft-gallery-file-name">
									<a href="javascript:;"
									   title="Download"><?php _e( 'Example-Gallery-Name' ); ?></a>
								</div>
	...
	public function ft_gallery_duplicate_post_add_duplicate_post_button() {
		...
		if ( $current_screen->post_type == 'ft_gallery' && $verify !== 'ft_gallery' ) {
			...
			<div id="ft-gallery-duplicate-action">
				<a href="<?php echo wp_nonce_url( 'admin.php?action=ft_gallery_duplicate_post_as_draft&post=' . $id, basename( __FILE__ ), 'duplicate_nonce' ); ?>"
				   title="Duplicate this item"
				   rel="permalink"><?php _e( 'Duplicate Gallery', 'feed-them-gallery' ); ?></a>
			</div>
			<?php
		}
	}
