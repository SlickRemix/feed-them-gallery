...
class Gallery {
	...
	public function ft_gallery_scripts() {
		...
		if ( is_admin() && $current_screen->post_type == 'ft_gallery' && $current_screen->base == 'post' ) {
            ...
            wp_register_style('side_sup_settings_css', plugins_url('feed-them-gallery/admin/css/metabox.css'), array(), FTG_CURRENT_VERSION);
			...
			wp_register_script( 'ft-gallery-metabox', plugins_url( 'feed-them-gallery/admin/js/metabox.js' ), array(), FTG_CURRENT_VERSION );
	 ...
	 * @since 1.0.0
	 */
	public function ft_gallery_uploader_meta_box( $object ) {
        ...
        echo '<textarea class="fts-gallery-description"></textarea><br/><div class="fts-gallery-tags-edit-wrap"></div><br/><div class="ft-submit-wrap"><a class="ft-gallery-edit-img-ajax button button-primary button-large" id="ft-gallery-edit-img-ajax" href="javascript:;" data-nonce="' . wp_create_nonce( 'ft_gallery_edit_image_nonce' ) . '">' . __( 'Save', $this->plugin_locale ) . '</a></div>';
			...
			<div class="tabs" id="tabs">
				<div class="tabs-menu-wrap" id="tabs-menu">
					<ul class="nav nav-tabs nav-append-content">
						...
						<li class="tabbed tab4">
							<a href="#zips" data-toggle="tab" aria-expanded="false">
								<div class="ft_icon"></div>
						...
						<li class="tabbed tab6">
							<a href="#watermark" data-toggle="tab" aria-expanded="false">
								<div class="ft_icon"></div>
								<span class="das-text"><?php _e( 'Watermark', 'feed-them-gallery' ); ?></span>
							</a>
						</li>

                        <li class="tabbed tab7">
                            <a href="#pagination" data-toggle="tab" aria-expanded="false">
                                <div class="ft_icon"></div>
                                <span class="das-text"><?php _e( 'Pagination', 'feed-them-gallery' ); ?></span>
                            </a>
                        </li>
					</ul>
				</div>
				...
				<div class="tab-content-wrap">
							...
							<div class="ftg-section">
								...
								<div id="uploadContainer" style="margin-top: 10px;">
									...
									if ( is_plugin_active( 'woocommerce/woocommerce.php' ) && is_plugin_active( 'feed-them-gallery-premium/feed-them-gallery-premium.php' ) && isset( $_GET['post'] ) ) {
										$auto_woo_pro_option = get_post_meta( $_GET['post'], 'ft_gallery_auto_image_woo_prod', true );
									...
									?>
									<!-- Uploader section -->
									<div id="uploaderSection"<?php echo isset( $ftg_auto_create_on_upload ); ?>>
                                    ...
                                    $post_count = '50';
                                    $orderby_set = null !== get_post_meta($object->ID, 'ftg_sort_type', true) ? get_post_meta($object->ID, 'ftg_sort_type', true) : 'menu_order' ;
									...
									if ( $multiple ) :
										?>
										plupload-upload-uic-multiple<?php endif; ?>" id="<?php echo $id; ?>plupload-upload-ui">
                                ...
                                $total_pagination_count = $this->ft_gallery_count_post_images($object->ID);
                                if($post_count < $total_pagination_count){
                                    ...
                                    print '<div class="ftg-images-amount-wrap"><select name="images" class="ftg-images-amount" onchange="javascript:reloadPage(this)">
                                            <option value="50"' . $orderby_ten . '>' . esc_html( '50 Images', 'feed-them-gallery' ) . '</option>
                                            <option value="100"' . $orderby_hundred . '>' . esc_html( '100 Images', 'feed-them-gallery' ) . '</option>
                                            <option value="500"' . $orderby_five_hundred . '>' . esc_html( '500 Images', 'feed-them-gallery' ) . '</option>
                                            <option value="-1"' . $orderby_all . '>' . esc_html( 'All Images (This may load slow if you have more than 500 images)', 'feed-them-gallery' ) . '</option>
                                            </select></div>';
                                    ...
                                    if('-1' !== $_GET['images']) {
                                        ...
                                        print '<div class="ftg-total-pagination-count">' . esc_html( 'Showing', 'feed-them-gallery' ) . ' ' . esc_html( $per_page_final ) . '-' . esc_html( $count_per_page ) . '  of ' . esc_html( $total_pagination_count ) . ' ' . esc_html( 'Images', 'feed-them-gallery' ) . '</div>';
                                    }
                                    print '<div class="ftg-pagination">';
								...
								if ( isset( $image_list[0] ) ) {
									...
									<div class="ft-gallery-options-buttons-wrap">
										...
										?>
										<div class="gallery-edit-button-wrap">
											<button disabled type="button" class="ft-gallery-zip-gallery ft_gallery_download_button_icon button button-primary button-larg" onclick="ft_gallery_create_zip('<?php echo $object->ID; ?>', 'no', 'yes', 'no')"><?php _e( 'Create Digital Gallery Product', 'feed-them-gallery' ); ?></button>
											<a class="gallery-edit-button-question-two" href="javascript:;" rel="gallery-edit-question-digital-gallery-product">?</a>
										</div>
										<div class="gallery-edit-button-wrap">
											<button type="button" disabled="disabled" class="ft-gallery-create-woo ft_gallery_download_button_icon button button-primary button-larg" onclick="ft_gallery_image_to_woo('<?php echo $this->parent_post_id; ?>')"><?php _e( 'Create individual Image Product(s)', 'feed-them-gallery' ); ?></button>
								...
								<div class="gallery-edit-question-message gallery-edit-question-digital-gallery-product" style="display: none;">
									...
									<?php
									echo sprintf(
										__( 'This button will create a zip on the %1$sZIPs tab%2$s of all the full size images in this gallery and then create a WooCommerce Product out of that ZIP. You must have a "ZIP Model Product" selected on the %3$sWoocommerce tab%4$s for this to work.', 'feed-them-gallery' ),
										'<a href="#zips" class="ftg-zips-tab" >',
										'</a>',
								...
								if ( $multiple ) :
									?>
									plupload-thumbs-multiple<?php endif; ?>" id="<?php echo $id; ?>plupload-thumbs" data-post-id="<?php echo $object->ID; ?>">
									<?php
									...
									if ( is_array( $image_list ) && isset( $image_list[0] ) ) {
										...
										foreach ( $image_list as $key => $image ) {
											...
											$meta_box = '<li class="thumb' . $product_created_already . '" id="list_item_' . $image['id'] . '" data-image-id="' . $image['id'] . '" data-menu-order="' . $image['menuOrder'] . '">';
					...
					<div class="tab-pane zips-tab-pane" id="zips">
						<div id="ftg-tab-content6" class="tab-content
						<?php
						if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'ft_zip_gallery' ) {
							echo ' pane-active';
						}
						?>
						">
					...
					<div class="tab-pane woocommerce-tab-pane" id="woocommerce">
						...
						">
							...
							<div class="tab-5-extra-options">
								...
								<div class="feed-them-gallery-admin-input-wrap"  style="border: none; padding-bottom:0px;">
									<div class="feed-them-gallery-admin-input-label"><?php _e( 'Use Smart Image Orientation', 'feed-them-gallery' ); ?></div>
									<?php
									if ( is_plugin_active( 'woocommerce/woocommerce.php' ) && is_plugin_active( 'feed-them-gallery-premium/feed-them-gallery-premium.php' ) ) {
										?>
									...
									?>
									<span class="tab-section-description"><small><?php _e( 'Checking this option will automatically determine the images orientation and match it to the appropriate Model Product (Landscape, Square or Portrait) when creating a WooCommerce product for each image. Check this option and then choose a "Smart Image Orientation Model Product" below. This will override the "Global Model Product" option for this Gallery.', 'feed-them-gallery' ); ?></small></span>
								...
								<div class="feed-them-gallery-admin-input-wrap ftg-zip-option-wrapper">
									...
									<span class="tab-section-description"><a href="https://docs.woocommerce.com/document/managing-products/#section-5" target="_blank"><small>
										 <?php
											echo sprintf(
												__( 'Learn how to create a %1$sSimple product%2$s in WooCommerce.', 'feed-them-gallery' ),
												'<strong>',
												'</strong>'
											);
											?>
								...
								<?php
								echo sprintf(
									__( 'Additional Global WooCommerce options available on the %1$sSettings Page%2$s', 'feed-them-gallery' ),
									'<a href="' . esc_url( 'edit.php?post_type=ft_gallery&page=ft-gallery-settings-page' ) . '" >',
									'</a>'
								);
								?>
...
?>
                ...
                <div class="tab-pane pagination-tab-pane" id="pagination">
                    <div id="ftg-tab-content8" class="tab-content
					<?php
                    if ( isset( $_GET['tab'] ) && $_GET['tab'] == 'ft_pagination' ) {
                        echo ' pane-active';
                    }
                    ?>
					">
                        ...
                        echo $this->ft_gallery_settings_html_form( $this->parent_post_id, $this->saved_settings_array['pagination'], null );
                        ?>
			...
			jQuery(document).ready(function ($) {
                ...
                jQuery('#ft_gallery_show_true_pagination').bind('change', function (e) {
                    if (jQuery('#ft_gallery_show_true_pagination').val() == 'yes') {
                        ...
                        jQuery('#ft_gallery_load_more_option').attr('disabled', 'disabled');
                        jQuery('.ftg-pagination-notice-colored').hide();
                        jQuery('.ftg-loadmore-notice-colored').show();
                    }
                    else {
                        jQuery('.ftg-pagination-options-wrap').hide();
                        jQuery('#ft_gallery_load_more_option').removeAttr('disabled');
                        jQuery('.ftg-loadmore-notice-colored').hide();
                    }
                });
                if (jQuery('#ft_gallery_show_true_pagination').val() == 'no') {
                    jQuery('.ftg-pagination-options-wrap').hide();
                    jQuery('#ft_gallery_load_more_option').removeAttr('disabled');
                }
                if (jQuery('#ft_gallery_show_true_pagination').val() == 'yes') {
				...
				jQuery('#ft_gallery_load_more_option').bind('change', function (e) {
					if (jQuery('#ft_gallery_load_more_option').val() == 'yes') {
                        ...
                        jQuery('#ft_gallery_show_true_pagination').attr('disabled', 'disabled');
                        ...
                        jQuery('#ft_gallery_show_true_pagination').removeAttr('disabled');
                        jQuery('.ftg-pagination-notice-colored').hide();
					}
				});
	...
	function ft_gallery_duplicate_post_link( $actions, $post ) {
		...
		if ( current_user_can( 'edit_posts' ) && $_GET['post_type'] == 'ft_gallery' ) {
			$actions['duplicate'] = '<a id="ft-gallery-duplicate-action" href="' . wp_nonce_url( 'admin.php?action=ft_gallery_duplicate_post_as_draft&post=' . $post->ID, basename( __FILE__ ), 'duplicate_nonce' ) . '" title="Duplicate this item" rel="permalink">' . __( 'Duplicate', 'feed-them-gallery' ) . '</a>';
		}
	...
	function ft_gallery_duplicate_post_add_duplicate_post_button() {
		...
		if ( $current_screen->post_type == 'ft_gallery' && $verify !== 'ft_gallery' ) {
			$id = $_GET['post'];
			?>
